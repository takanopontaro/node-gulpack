...
var Task = (function (_require) {
  ...
  _createClass(Task, null, [{
    ...
    value: function register(conf) {
      ...
      var sourcemap = _ins$conf.sourcemap;
      var autoprefixer = _ins$conf.autoprefixer;

      _gulp2['default'].task(name, preTasks, function () {
        return _gulp2['default'].src(glob).pipe($['if'](ins.useCache, $.cached(name))).pipe($.plumber({ errorHandler: ins.plumberHandler(name) })).pipe($['if'](sourcemap, $.sourcemaps.init())).pipe($.sass(options)).pipe($['if'](!!autoprefixer, $.autoprefixer(ins.toOptions(autoprefixer)))).pipe($['if'](sourcemap, $.sourcemaps.write('.'))).pipe(_gulp2['default'].dest(dest));
      });
      return ins;
    }
  }]);
