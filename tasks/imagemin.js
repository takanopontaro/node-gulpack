  ...
  _createClass(Task, null, [{
    ...
    value: function register(conf) {
      ...
      _gulp2['default'].task(name, preTasks, function () {
        return _gulp2['default'].src(glob).pipe($.plumber({ errorHandler: ins.plumberHandler(name) })).pipe($.changed(dest)).pipe($.imagemin(options)).pipe(_gulp2['default'].dest(dest));
      });
      return ins;
    }
  }]);
