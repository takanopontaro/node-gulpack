...
var Task = (function (_require) {
  ...
  _createClass(Task, null, [{
    ...
    value: function register(conf) {
      ...
      var extension = _ins$conf.extension;
      var options = _ins$conf.options;

      _gulp2['default'].task(name, preTasks, function () {
        return _gulp2['default'].src(glob).pipe($['if'](ins.useCache, $.cached(name))).pipe($.ignore.exclude(/\/_[^\/]+jade$/)).pipe($.plumber({ errorHandler: ins.plumberHandler(name) })).pipe($.jade(options)).pipe($.rename(function (path) {
