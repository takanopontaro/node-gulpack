...
var Task = (function (_require) {
  ...
  _createClass(Task, null, [{
    ...
    value: function register(conf) {
      ...
      var consolidate = _ins$conf.consolidate;
      var css = _ins$conf.css;

      _gulp2['default'].task(name, preTasks, function () {
        return _gulp2['default'].src(glob).pipe($.plumber({ errorHandler: ins.plumberHandler(name) })).pipe($.iconfont(options)).on('glyphs', function (glyphs, options) {
          var info = _path2['default'].parse(css.dest);
          Object.assign(consolidate, {
            glyphs: glyphs,
            fontName: options.fontName
          });
          _gulp2['default'].src(css.tmpl).pipe($.consolidate('lodash', consolidate)).pipe($.rename({ basename: info.name })).pipe(_gulp2['default'].dest(info.dir));
