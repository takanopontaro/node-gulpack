  ...
  _createClass(_default, null, [{
    ...
    value: function task(conf) {
      ...
      _gulp2['default'].task(name, preTasks, function () {
        return _gulp2['default'].src(glob).pipe($['if'](ins.useCache, $.cached(name))).pipe($.ignore.exclude(/\/_[^\/]+jade$/)).pipe($.plumber({ errorHandler: ins.plumberHandler(name) })).pipe($.jade(options)).pipe(_gulp2['default'].dest(dest));
      });
      return ins;
    }
  }]);
