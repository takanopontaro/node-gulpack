...
var _default = (function (_require) {
  ...
  _createClass(_default, null, [{
    ...
    value: function task(conf) {
      ...
      var uglify = _ins$conf.uglify;

      _gulp2['default'].task(name, preTasks, function () {
        return _gulp2['default'].src(glob).pipe($.plumber({ errorHandler: ins.plumberHandler(name) })).pipe($['if'](sourcemap, $.sourcemaps.init())).pipe($.babel(options)).pipe($['if'](sourcemap, $.sourcemaps.write('.'))).pipe($['if'](!!uglify, $.uglify(ins.toOptions(uglify)))).pipe(_gulp2['default'].dest(dest));
      });
      return ins;
    }
  }]);
